...
function Cadastro() {
  ...
  const handleCadastro = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        localStorage.setItem("usuario", JSON.stringify({ nome, foto: result.foto }));
        navigate("/feed");
      } else {
    ...
    <main className="bg-gradient-to-b from-blue-100 to-purple-100 h-screen w-full flex flex-col items-center">
      ...
      <div className="bg-zinc-50 w-85 h-130 rounded-xl shadow-lg p-6 flex flex-col pt-5">
        ...
        <form className="flex flex-col mt-4 gap-1" onSubmit={handleCadastro}>
          ...
          <input type="text" placeholder="Digite seu Login" value={nome} onChange={e => setNome(e.target.value)}
          ...
          <input type="password" placeholder="********" value={confirmaSenha} onChange={e => setConfirmaSenha(e.target.value)}
            className="border text-sm border-zinc-300 rounded-md p-3 mb-4 pl-2 focus:border-purple-500 focus:outline-none"/>
